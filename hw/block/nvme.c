...
static void lnvm_print_lba(LnvmCtrl *ln, uint64_t lba)
{
    ...
    printf("phys: 0x%016lx; {pugrp: %u, punit: %u, chunk: %u, sectr: %u}\n",
...
static inline int lnvm_lba_to_chunk_no(LnvmCtrl *ln, uint64_t lba)
{
    ...
    if (chk >= ln->params.chk_per_lun ||
            ...
            ch >= ln->params.num_ch) {
        printf("nvme: accessing unmapped chunk: ch:%lu, lun:%lu, chk:%lu, cno: %lu\n", ch, lun, chk, cno);
        return -1;
    }
    ...
    NvmeNamespace *ns, uint64_t slba, uint32_t nlba)
{
    ...
    if (cnk->state & LNVM_CHUNK_BAD || cnk->state & LNVM_CHUNK_CLOSED) {
        printf("lvnm_rw_check_chunk_write: write fault"
               " (chunk state: 0x%02x)\n  ", cnk->state);
    ...
    if (end_sectr > cnk->cnlb) {
        printf("lvnm_rw_check_chunk_write: out of bounds write"
               " (sectr: %d, cnlb: %ld)\n  ", end_sectr, cnk->cnlb);
    ...
    if (start_sectr != cnk->wp) {
        printf("lvnm_rw_check_chunk_write: out of order write"
               "  (sectr: %d, wp: %ld)\n  ", start_sectr, cnk->wp);
        lnvm_print_lba(ln, slba);
    ...
    uint32_t nlb)
{
        ...
        for (int j = 0; j < max_chunks_per_req; j++) {
            if (m[j].ws) {
                ...
                if (m[j].chunk_slba.v == chunk_slba.v) {
                    ...
                    if (++(m[j].lbap) != psl[i]) {
                        printf("lvnm_rw_check_write_req: out of order write"
                               "\n");
                        printf("  tried: "); lnvm_print_lba(ln, psl[i]);
                        printf("  last : "); lnvm_print_lba(ln, m[j].lbap-1);
            ...
            if (j == max_chunks_per_req-1) {
                ...
                printf("lnvm_rw_check_write_req failed: exceeded max number of"
    ...
    for (int i = 0; i < max_chunks_per_req; i++) {
        ...
        if (m[i].ws < wrt->ws_min && (m[i].ws % wrt->ws_min != 0)) {
            fprintf(stderr, "lnvm_rw_check_write failed: request does not "
                   "respect device write constraints (ws: %d, ws_min: %d)\n  ",
                   m[i].ws, wrt->ws_min);
...
static int lnvm_chunk_set_free(NvmeNamespace *ns, LnvmCtrl *ln, uint64_t lba, hwaddr mptr)
{
    ...
    if (!(chunk_meta->state & LNVM_CHUNK_CLOSED) ||
                      (chunk_meta->state & (LNVM_CHUNK_OPEN ||
                                            ...
                                            LNVM_CHUNK_FREE))) {
        if (chunk_meta->state & LNVM_CHUNK_FREE) {
            printf("nvme: double reset\n  ");
            lnvm_print_lba(ln, lba);
        } else if (chunk_meta->state & LNVM_CHUNK_OPEN) {
            printf("nvme: early reset (wp: %" PRIu64 ")\n  ", chunk_meta->wp);
            lnvm_print_lba(ln, lba);
	    } else {
            printf("nvme: invalid chunk state during reset (wp: %" PRIu64 "))\n", chunk_meta->wp);
            lnvm_print_lba(ln, lba);
            printf("  state: %d\n", chunk_meta->state);
        }
    }
    ...
    NvmeRequest *req)
{
    ...
    if (req->is_write) {
        if (nlb < wrt->ws_min) {
            printf("lnvm_rw_check_write failed: request does not respect "
    ...
    aio_slba =
            ns->start_block + (req->slba << (data_shift - BDRV_SECTOR_BITS));
    ...
    NvmeRequest *req)
{
    ...
    err = lnvm_rw_setup_rq(n, ns, lrw, &aio_sector_list, req, psl,
                                                            data_shift, nlb);
    if (err) {
        printf("lnvm_rw: lnvm_rw_setup_rq failed: 0x%x\n  ", err);
